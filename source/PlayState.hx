...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		bubbles = new FlxBackdrop(Paths.image('bubbles'), 0.2, 0, true, true);
		bubbles.velocity.set(0, -350);
		bubbles.updateHitbox();
		bubbles.screenCenter(XY);
		...
		switch (curStage)
		{
			case 'universe': //Week 1
				...
				stage = new BGSprite('stage', -250, -80, 0.9, 0.9);
				stage.alpha = 0;
				stage.scale.set(1.20, 1.20);

				add(stage);

				spooky = new BGSprite('spooky', -250, -80, 0.9, 0.9);
				spooky.alpha = 0;
				...
				snow = new BGSprite('snow', -250, -80, 0.9, 0.9);
				snow.alpha = 0;
				snow.scale.set(1.20, 1.20);

				add(snow);

				tank = new BGSprite('tank', -250, -80, 0.9, 0.9);
				tank.alpha = 0;
				tank.scale.set(1.20, 1.20);
		...
		switch(SONG.player1) {
			case 'erbf':
				GameOverSubstate.characterName = "erbf-death";
		}
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			var babyArrow:StrumNote = new StrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLineY, i, player);
	...
	function sortByBeat(Obj1:BeatEvent, Obj2:BeatEvent):Int
	{
	...
	{
		beatEvents.push(new BeatEvent(beat, callback));
		beatEvents.sort(sortByBeat);
	}

	public var songLengthOffset:Float = 0;
	...
	override public function update(elapsed:Float)
	{
		...
		while(beatEvents.length > 0 && curBeat >= beatEvents[0].beat) {
			var event = beatEvents.shift();
			event.callback();
		}
		...
		else
		{
			...
			if (!paused)
			{
				if(updateTime) {
					...
					songPercent = (curTime / (songLength + songLengthOffset));

					if(ClientPrefs.timeBarType != 'Song Name') {
						var songCalc:Float = ((songLength + songLengthOffset) - curTime);
	...
	function setupStepEvents() {
		if (formattedSong == 'euthanasia-rollercoaster' && curStage == 'universe') {
			pushStepEvent(8, () -> {
				FlxTween.cancelTweensOf(blacksquare);
				FlxTween.cancelTweensOf(bubbles);
				FlxTween.tween(blacksquare, {alpha: 0}, 1);
				FlxTween.tween(bubbles, {alpha: 0.6}, 1);
			});
			pushStepEvent(305, () -> {
	...
	function setupBeatEvents() {
		if (formattedSong == 'euthanasia-rollercoaster' && curStage == 'universe') {
			...
			pushStepEvent(480, () -> {
				...
				FlxTween.tween(stars, {alpha: 1}, 2, {startDelay: 6});
			});
			pushStepEvent(560, () -> {
				FlxTween.cancelTweensOf(stars);
				FlxTween.tween(stars, {alpha: 0}, 2);
	...
	override function beatHit()
	{
		...
		/*if (formattedSong == 'euthanasia-rollercoaster' && curStage == 'universe') {//ughgoddamn
			switch (curBeat)
			{
				case 1:
				{
...
class BeatEvent 
{
	...
	public function new(daBeat:Int = 0, daCallback:Void->Void = null)
	{
		beat = daBeat;
		callback = daCallback;
	}
